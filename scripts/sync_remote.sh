#!/usr/bin/env bash
# Sync project to remote server via rsync, then optionally restart the API server.
#
# Usage:
#   ./scripts/sync_remote.sh              # sync only
#   ./scripts/sync_remote.sh --restart    # sync + uv sync + restart API server (systemctl)
#
...
REMOTE_HOST="${REMOTE_HOST:-sku-match-jihai-gpu}"
REMOTE_DIR="${REMOTE_DIR:-/root/sku-match}"
